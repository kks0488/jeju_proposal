...
intro='하백/이아컴퍼니와 GAU가 78점으로 공동 1위다. 현재 자료로 둘 사이에 우열을 줄 근거가 부족하다. 3~6위는 JEJUPH 54점, 시스홈 52점, 이음 50점, 포엠 48점으로 근접해 있어 실제 소장·공사범위 확인에 따라 바뀔 수 있다. 우선 면담은 하백·GAU, 성능 비교는 JEJUPH가 합리적이다. 시스홈은 점수와 별개로 기존 RC 설계를 맡을 제주팀 확인이 먼저다.'
notes=[
...
h+=table(sh,sv)
sens='하백·GAU 공동 선두는 세 배점안 모두 유지된다. 반면 포트폴리오를 40점으로 높이면 시스홈 61점이 JEJUPH 54점을 앞선다. 제주·RC를 강화하면 이음과 JEJUPH가 54점으로 같아진다. 따라서 중간 순위를 실력의 확정 서열로 읽으면 안 된다. 기본안은 현재 설계 유지와 제주 수행을 함께 반영했다.'
