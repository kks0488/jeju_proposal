...
intro='하백/이아컴퍼니와 GAU는 78점으로 공동 1위를 유지한다. 다봄은 RC 반복 실적과 거주 기록을 추가해 57점·4위로 수정하되, 소원재 공정 불만과 계약 주체 미확인으로 조건부다. 아틀리에건설은 60점으로 3위에 추가하되, 서울 기반이고 현재 제주 상주팀이 확인되지 않은 조건부 비교 대상이다. 제주 현지 우선 검증은 하백·GAU, 성능 비교는 JEJUPH 54점으로 유지한다. 점수는 계약 적격이나 업계 실력 순위가 아니다. 상세 추가 조사와 구조 구분은 review.html#expanded에서 확인할 수 있다.'
notes=[
