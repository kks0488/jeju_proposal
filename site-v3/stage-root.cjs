...
fs.cpSync(path.join(ROOT,'research'),path.join(OUT,'research'),{recursive:true});
...
const status={edition:'ranking-main-20260915',title:d.title||'제주집 시공사 선정',projects:d.projects.length,images:d.projects.reduce((n,p)=>n+p.photos.length,0),research:JSON.parse(fs.readFileSync(path.join(ROOT,'research/catalog.json'),'utf8')).stats,delivery:'static-local-assets',builtAt:new Date().toISOString()};
